 ...
 * }
 */
final class Shurloc_User_Cart_Migration {
	 ...
	 * Constructor.
	 *
	 * @param Shurloc_User_Cart_Service $cart_service Cart service.
	 */
	public function __construct(
		Shurloc_User_Cart_Service $cart_service
	) {
	...
	public function run(): array {
		...
		foreach ( $rows as $row ) {
			...
			try {
				...
				$contents_total =
					$this->get_contents_total(
						session: $session,
						cart_contents: $cart_contents,
					);

				$stored =
					$this->cart_service->store_cart_snapshot(
						user_id: $user_id,
						cart_contents: $cart_contents,
		...
		foreach ( $stored_cart as $item_key => $item ) {
			...
			$cart_contents[] = array(
				...
				'line_total'    => isset( $item['line_total'] )
					...
					: 0.0,
				'variation'     =>
					$this->normalize_variation_attributes(
						item: $item,
					),
			);
		}
