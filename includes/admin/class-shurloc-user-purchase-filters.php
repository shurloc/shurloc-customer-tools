 ...
 */
final class Shurloc_User_Purchase_Filters {
...
<!--
		...
		<select
			name="<?php echo esc_attr( self::LAST_ORDER_STATUS_FILTER ); ?>"
			id="<?php echo esc_attr( self::LAST_ORDER_STATUS_FILTER ); ?>"
		>
			<option value="">
				<?php
				echo esc_html__(
					'All Last Order Statuses',
					'shurloc-customer-tools'
				);
				?>
			</option>
			...
			<?php foreach ( $this->get_order_statuses() as $status => $label ) : ?>
				...
				<option
					...
					<?php selected( $last_order_status_filter, $status ); ?>
				>
...
-->
	...
	private function apply_sorting(
		...
		if (
			Shurloc_User_Purchase_Service::LAST_PURCHASE_META_KEY !== $orderby
		) {
			return;
		}
	...
	private function build_purchase_meta_query_clause(
		...
		$filter_parameters = $this->get_purchase_filter_parameters( $filter );
	 ...
	 * the keys before rendering and validating the filter.
	 *
